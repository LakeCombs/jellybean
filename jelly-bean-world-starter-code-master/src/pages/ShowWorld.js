...
import {
  getWorld,
  deleteWorld,
  updateJellyBeanCount,
} from "../services/JellyBeanAPIService"; //Node Server Enpoint Service
...
class ShowWorld extends Component {
  ...
  async componentDidMount() {
    ...
    if (propsLocationState) {
      ...
      this.setState({ worldName: worldName, worldData: worldDoc.data }, () =>
        this.showWorld()
      );
    }
  }
  ...
  async updateJellyBeanCount() {
    ...
    if (this.state.inputtedNumOfJellyBeans.length > 0) {
      var requestResponse = await updateJellyBeanCount(
        this.state.worldName,
        this.state.inputtedNumOfJellyBeans
      );
  ...
  async showWorld() {
    ...
    jellyBeanInterval = setInterval(() => {
      if (jellyBeansLeftToSpawn > 0) {
        ...
        var jellyBean = Bodies.fromVertices(
          Common.random(0, 600),
          worldData.jellyBeanSpawnY,
          vertexSets,
          {
            render: {
              fillStyle: color,
    ...
    return (
      <Container style={styles.viewContainer}>
        ...
        <Col style={styles.inputSection}>
          <div style={styles.inputPrompt}>
            Enter a number below to change the amount of beans that spawn into
            the world
          </div>
          ...
          <Row style={styles.inputButtonRow}>
            <TextField
              ...
              onChange={(event) =>
                this.setInputtedNumOfJellyBeans(event.target.value)
              }
              onKeyPress={(event) => {
                if (event.key === "Enter") {
        ...
        <Row style={styles.bottomButtonRow}>
          <Button
            style={{
              ...styles.jellyBeanButton,
              ...styles.jellyBeanButtonOrange,
            }}
          ...
          <Button
            style={{
              ...styles.jellyBeanButton,
              ...styles.jellyBeanButtonYellow,
            }}
            onClick={() => this.showSelectWorld()}
          >
            <div style={styles.jellyBeanButtonText}>Go Back</div>
          </Button>

          <Button
            style={{ ...styles.jellyBeanButton, ...styles.jellyBeanButtonGray }}
